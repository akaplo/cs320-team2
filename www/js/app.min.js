...
app.controller('DashCtrl', function ($scope, sqlService, $ionicPlatform) {
  $ionicPlatform.ready(function () {
    ...
    sqlService.init().then(function (res) {
      // run a view query
      sqlService.viewTable('feedback').then(function (result) {
        return console.log("View result", result.rows.item(1));
      }, function (err) {
...
app.controller('PreferencesCtrl', function ($scope, sqlService, $ionicPlatform) {
  $ionicPlatform.ready(function () {
    ...
    sqlService.init().then(function (res) {
      // run a view query
      sqlService.viewTable('preferences_table').then(function (result) {
        return console.log("View result", result, result.rows.item(0));
      }, function (err) {
        return console.log("View error", err);
      });
    });
...
app.controller('LineCtrl', function ($scope) {
  ...
  $scope.data = [[0, 5, 7, 9, 6, 7, 8], [0, 3, 3, 5, 4, 5, 5]];
});
'use strict';

/* jshint esversion:6 */
app.controller('PatternsCtrl', function ($scope, ViewPatterns) {
  return ViewPatterns.all().then(function (patterns) {
...
app.factory('sqlService', function ($cordovaSQLite) {
	...
	popQrys.mood_logs = ['DROP TABLE IF EXISTS mood_logs', 'CREATE TABLE mood_logs(id INTEGER PRIMARY KEY NOT NULL,\
	...
	popQrys.feedback = ['DROP TABLE IF EXISTS feedback', 'CREATE TABLE feedback( copingStrategy TEXT PRIMARY KEY, response INTEGER NOT NULL )', 'INSERT INTO feedback (copingStrategy, response) VALUES\
		("Watch Spongebob", 1),\
		("Go to the gym", 0),\
	...
	popQrys.preferences_table = ['DROP TABLE IF EXISTS preferences_table', 'CREATE TABLE preferences_table(\
		id INTEGER PRIMARY KEY NOT NULL,\
		name TEXT NOT NULL,\
		password TEXT NOT NULL,\
		contact TEXT NOT NULL,\
		backgroundURL TEXT NOT NULL,\
		reminderRate INTEGER NOT NULL DEFAULT 86400000)', 'INSERT INTO preferences_table (name, password, contact, backgroundURL, reminderRate) VALUES (\'John\', \'password123\' ,\'idk@idk.com\', \'http://vignette2.wikia.nocookie.net/thehungergames/images/4/48/Happy_cat.jpg/revision/latest?cb=20121008044759\', 86400000)'];

	var populate = function populate() {
		return new Promise(function (resolve, reject) {
			if (db === null) reject("DB connection not initiated. Call init() before running queries.");

			db.sqlBatch([].concat(_toConsumableArray(popQrys.pattern_features), _toConsumableArray(popQrys.mood_logs), _toConsumableArray(popQrys.feedback), _toConsumableArray(popQrys.preferences_table)), function (error) {
				if (error) reject(error);
				// just some test data
				db.executeSql('INSERT INTO mood_logs (mood, intensity, trigger, behavior, belief) VALUES (\'my mood\', 7, \'some trigger\', \'some behavior\', \'some belief\')', [], function (resultSet) {
					resolve();
				}, function (error) {
					return reject(error);
				});
